...
- 普通任务只在自己的 worktree 内使用已忽略的 `DerivedData/`；不得在长期保留的 `main` 工作区创建 `.codex-derived-*`、`.derived-data-*` 或把 DerivedData 放进 `artifacts/`。`artifacts/` 只用于需要人工查看的截图、日志摘要等验收证据，任务 worktree 验收后连同临时构建产物一起删除。
- 标准真机安装流程会在安装成功后运行 `ci/prune-ios-install-deltas.sh --apply`，只为 `BUNDLE_ID` 保留最新 3 份安装增量。调试时可临时设置 `IOS_PRUNE_INSTALL_DELTAS=false`，但不得把它设为 Runner 的持久配置。
